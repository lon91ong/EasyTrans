...
# 翻译文献到新的pdf以及word中
path = root + r"\trans\input_file\test.pdf"#这里改pdf的名字
...
try:
    for cur_page in cur_pdf:
        ...
        for img in img_list:  # 获取当前页面的图像列表
            ...
            fout.close()
            bf = None  # 释放资源
        blks = cur_page.getTextBlocks(flags = 4)  # read text blocks of input page
        ...
        for num in range(len(blks)):  # loop through the blocks
            ...
            if blks[num][-1] == 1:
                ...
                try:
                    path_img = os.path.join(root, 'trans', 'output_file',
            ...
            if num < len(blks) - 1:
                ...
                if (abs(blks[num + 1][1] - blks[num][3]) <= 1.0 and abs(
                    ...
                    if reference_flag == 1:
                        ...
                        new_page.insertTextbox(r, res, fontname="song", fontfile=os.path.join(root,
                ...
                # 两个块y轴距离远的的时候
                    ...
                    if flag == 1:
                        ...
                        # 标记在这里之后的都是参考文献
                            ...
                            reference_flag = 1
                            new_page.insertTextbox(r, '参考文献', fontname="song", fontfile=os.path.join(root,
                                                                                               'trans/static/fonts/SimSun.ttf'),
                                                fontsize=fonts, align=text_pos)
                        else:
                            # 翻译结果去掉汉字中的空格
    ...
    print(new_file_name)
    new_docx_name = os.path.join(root, 'trans', 'output_file',
